 ...
 * 3.140592653839794 for PI at the end of 1000 iterations : since you are within
 * epsilon of Math::PI you return
 * 
 * iter_pi(0.001) --> [1000, 3.1405926538]
 * Notes :
 * Unfortunately, this series converges too slowly to be useful, as it takes
 * over 300 terms to obtain a 2 decimal place precision. To obtain 100 decimal
 * places of PI, it was calculated that one would need to use at least 10^50
 * terms of this expansion!
 * 
 * About PI : http://www.geom.uiuc.edu/~huberty/math5337/groupe/expresspi.htmlF
 */

public class PiApprox {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        double epsilon = Double.valueOf(sc.nextLine());
    ...
    public static String iterPi2String(double epsilon) {
        double current = 0;
        int index = 1;
        while (!(current * 4 > Math.PI - epsilon && current * 4 < Math.PI + epsilon)) {
            int a = 1 + (index - 1) * 2;
            if (index % 2 != 0) {
                current += 1.0 / a;
            } else {
        ...
        return String.format("[%d, %.10f]", index - 1, current * 4);
    }
}
