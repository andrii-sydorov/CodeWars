...
public class Rainfall {
	...
	public static String data = "Rome:Jan 81.2,Feb 63.2,Mar 70.3,Apr 55.7,May 53.0,Jun 36.4,Jul 17.5,Aug 27.5,Sep 60.9,Oct 117.7,Nov 111.0,Dec 97.9"
			+ "\n"
	...
	public static String data1 = "Rome:Jan 90.2,Feb 73.2,Mar 80.3,Apr 55.7,May 53.0,Jun 36.4,Jul 17.5,Aug 27.5,Sep 60.9,Oct 147.7,Nov 121.0,Dec 97.9"
			+ "\n"
	...
	private static List<Double> getListOfDouble(String town, String strng) {
		List<Double> ls = new ArrayList<>();
		String[] data = strng.split("\n");
		for (int i = 0; i < data.length; i++) {
			if (data[i].startsWith(town + ":")) {
				String temp = data[i].substring((town + ":").length());
				String[] arrMonth = temp.split(",");
				for (int j = 0; j < arrMonth.length; j++) {
					String[] monthAndTemperature = arrMonth[j].split(" ");
					ls.add(Double.valueOf(monthAndTemperature[1]));
				}
			}
		}
		return ls;
	...
	public static double variance(String town, String strng) {
		...
		double sum = 0;
		for (int i = 0; i < ls.size(); i++) {
			sum += Math.pow(ls.get(i) - meanValue, 2);
		}
		double beforeRoot = sum / ls.size();
		return beforeRoot;
	}

	/*
	 * public static double mean(String town, String strng) { 
	 * return parseTemp(town, strng).stream() .collect(averagingDouble(n -> n)); 
	 * }
	 * 
	 * public static double variance(String town, String strng) { 
