 ...
 * 
 * But at the end of the day you realise not everybody did the same amount of
 * work.
 * 
...
public class Dinglemouse {
    public static void main(String[] args) {
        ...
        sc.close();
        System.out.println(Arrays.toString(paintLetterboxes(start, end)));
    }

    public static int[] paintLetterboxes(int start, int end) {
        int[] ans = new int[10];
        while (start <= end) {
            fillArray(ans, start);
            start++;
    ...
    public static void fillArray(int[] ans, int number) {
        while (number != 0) {
            ans[number % 10]++;
            number /= 10;
        }
