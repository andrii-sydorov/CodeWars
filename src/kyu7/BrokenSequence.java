 ...
 * broken", e.g. "1 2 4 3")
 * 1 if there are any non-numeric elements in the input ("invalid", e.g. "1 2
 * a")
...
public class BrokenSequence {
    ...
    public static int findMissingNumber(String sequence) {
        ...
        if (sequence.replaceAll("[A-Za-z]", "").length() != sequence.length()
                || sequence.replaceAll("\\w", "").length() == 0) {
            return 1;
        }
        int[] arr = Arrays.stream(sequence.split(" ")).mapToInt(Integer::valueOf).toArray();
        Arrays.sort(arr);
