 ...
 * type words was called "multi-tap" and involved pressing a button repeatedly
 ...
 * 
 * In order to send the message "WHERE DO U WANT 2 MEET L8R" a teen would have
 * to actually do 47 button presses. No wonder they abbreviated.
 * 
 * For this assignment, write a module that can calculate the amount of button
 * presses required for any phrase. Punctuation can be ignored for this
...
public class Keypad {
	public static void main(String[] args) {
		...
		System.out.println(presses(phrase));
		sc.close();
	}

	public static int presses(String phrase) {
		int ans = 0;
		String[] keyPad = new String[12];
		...
		for (int i = 0; i < phrase.length(); i++) {
			for (int j = 0; j < keyPad.length; j++) {
				if (keyPad[j].contains(Character.toString(phrase.charAt(i)))) {
					ans += keyPad[j].indexOf(phrase.charAt(i)) + 1;
					break;
				} else {
					continue;
				}
