...
public class EqualSidesOfAnArray {
	public static void main(String[] args) {
		...
		String[] strArr = sc.nextLine().replaceAll(",", "").split(" ");
//        creating array of integer
	...
	public static boolean sumArray(int[] arr, int index) {
		int sumLeft = 0;
		int sumRight = 0;
 ...
 * 
 * public class Kata { public static int findEvenIndex(int[] arr) { 
 * return IntStream.range(0, arr.length) .filter(n -> IntStream.of(arr).limit(n).sum()
 * == IntStream.of(arr).skip(n + 1).sum()) .findFirst().orElse(-1); 
 * } 
 * }
 */
