 ...
 * series of letters into two words (eg. "elephant" and "ride"). The abbreviated
 * version of the word should have the first letter, then the number of removed
...
public class Word_a10n {
	public static void main(String[] args) {
		...
		sc.close();
		System.out.println(abbreviate(in));
	}
	...
	public static String abbreviate(String string) {
		...
		for (int i = 0; i < chArr.length; i++) {
			if (Character.isLetter(chArr[i])) {
				temp += Character.toString(chArr[i]);
			} else {
				if (temp.length() <= 3) {
					res += temp;
				} else {
					res += Character.toString(temp.charAt(0)) + (temp.length() - 2)
							+ Character.toString(temp.charAt(temp.length() - 1));
				}
				res += Character.toString(chArr[i]);
				temp = "";
			}
		}
		if (temp.length() <= 3) {
